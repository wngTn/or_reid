...
class SetBlockWrapper(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if len(x.size()) == 5:
            ...
            n, c, s, t = x.size()
            x = self.forward_block(x.transpose(
                1, 2).reshape(-1, c, t), *args, **kwargs)
            # We return xyz coordinates
            if is_list_or_tuple(x):
                return [_.reshape(n, s, *_.size()[1:]).transpose(1, 2).contiguous() for _ in x]
            output_size = x.size()
            return x.reshape(n, s, *output_size[1:]).transpose(1, 2).contiguous()
        else:
            raise ValueError(
                "Unsupported input with shape: {}".format(x.size()))
...
class SeparateFCs(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm:
            ...
            # x.shape = [p, n, c_in]
            # fc_bin.shape = [p, c_in, c_out]
            # out.shape = [p, n, c_out]
            # each part has its own fc_bin
