...
class DataSet(tordata.Dataset):
    def __init__(self, data_cfg, training):
        ...
        Args:
            data_cfg: Data configuration dict with dataset_root, dataset_partition, etc.
        ...
        # Mask the RGB frames
        self.masked = data_cfg['masked']

        self.num_sample = data_cfg.get('num_sample', None)
        self.class_num = 250
        ...
        for pth in paths:
            if pth.endswith('.pkl'):
                with open(pth, 'rb') as f:
                    _ = pickle.load(f)
                    _ = [array.astype(np.float32) for array in _]
                    # We have RGB Image, so we need to transpose it to (C, H, W)
                    if _[0].ndim > 2 and _[0].shape[2] == 3:
                        _ = np.array([np.transpose(array, (2, 0, 1)) for array in _])
                    if _[0].ndim > 2:
                        _ = np.array(_)
                        
                if self.masked:
                    rgb_filename = pth.split('/')[-1]
                    rgb_filename = "14" + rgb_filename[2:].replace("RGB_raw", "Sils_aligned")
                    with open(os.path.join(os.path.dirname(pth), rgb_filename), 'rb') as f:
                        # (nf, H, W)
                        masks = pickle.load(f).astype(np.uint8)
                        masks = masks[:, None, ...].repeat(3, axis=1)
                        if masks.any():
                            _[masks == 0] = 0
            else:
    ...
    def __dataset_parser(self, data_config, training):
        ...
        data_in_use = data_config.get('data_in_use', None)
        ...
        label_list = [label for label in label_list if int(label) < 1028]
